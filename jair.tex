...
In \cite{Beaumont_OCTRP_2001}, partial constraint satisfaction techniques were
applied to find a subset of satisfiable constraints. Later, disjunctive
...
introduced in \cite{Khatib01temporalconstraint}. Most of the prior work has
...
proceedings of prior IJCAI \cite{Yu_BCDR_2013}, ICAPS
\cite{Yu_CDRU_2014,cui2015optimising} and AAAI \cite{Yu_AAAI_2015} conferences.
...
\begin{itemize}
	...
	\item Simple temporal constraints where the duration between lower and upper
...
\begin{table}[h!]
	...
	\begin{tabular}{| p{3cm} | p{1.8cm} | p{3cm} | p{1.8cm} | p{3cm} |}
		\hline
		\textbf{Relaxation 1} & \multirow{6}{*}{\parbox{2cm}{\textbf{Do not\\ relax $C_{17}$}}} & \textbf{Relaxation 2} & \multirow{6}{*}{\parbox{2cm}{\textbf{$C_{2}$ is at\\ least 44}}} & \textbf{Relaxation 3} \\
		\hline
		$AM \leftarrow B$ &  & $AM \leftarrow B$ & & $AM \leftarrow B$ \\
...
Temporal problems with choices are usually modeled using Conditional Temporal
...
adding uncontrollable discrete choices and by conditioning the occurrence of
...
The solution to a CTP is a schedule that assigns a real value to each event in
the CTP and is consistent with all the temporal constraints. Conditional
Temporal Problems with Preferences (CTPPs,\cite{Falda07dynamicconsistency})
...
variables (choices) are controllable. Consequently, to determine the consistency
...
\begin{mydef}
	...
	\begin{itemize}
		...
		\item $P$ is a set of controllable finite domain discrete variables;
		\item $Q$ is the collection of assignments to $P$;
		\item $V$ is a set of events representing designated time points;
		\item $E$ is a set of temporal constraints between pairs of events $v_i\in V$;
...
relaxations. The motivation of this assumption is that people generally prefer
to minimize the perturbations to the original requirements, and penalize larger
...
on choices. The Temporal Networks with Alternatives (TNA) formulation introduced
by \cite{bartak2007temporal} also address this issue by introducing the concepts
...
Conditional Simple Temporal Problem (OCSTP) formulation introduced by
...
[l_{xy},b_{xy}]$ where $v_x, v_y$ are events. 
		\item uncertain durations $u_{xy} \in E_u$, where $u_{xy} \in [l_{xy},b_{xy}]$ describes the difference in time
		 between received event $r_y\in V_r$ and activated event $v_x\in V_a$, such that $(r_y - v_x) = u_{xy}$. 
...
set of all activated events; 
		\item $E_u\subseteq E$ is a set of uncertain duration between pairs of
...
\begin{itemize}
	...
	controllable, for which a dynamic execution strategy that meets all constraints can be derived. 
...
constraints. Here, we first repeat the definition of cc-pSTP from
\cite{Fang_AAAI_2014} for reference.
...
Given a cc-pSTP, $P$, if we execute the
controllable events using $S_x$ in its solution, the chance of violating any
temporal constraints in $P$ is guaranteed to be less than $\Delta_t$. The policy
...
\begin{mydef}
	...
	A valid resolution for an over-constrained cc-pSTP, $P$, is a 3-tuple $\langle
	R_e,\Delta'_t, N_{alloc}\rangle$, where:
	
	\begin{itemize}
		
		\item $R_e$ is a set of relaxations (in terms of relaxed lower and upper
		bounds) to constraints in $RE$ of $P$. 
		
		\item $\Delta'_t$ is a relaxation for $\Delta_t$ of $P$,
		and $\Delta'_t \geq \Delta_t$.
		
		\item $N_{alloc}$ is a STNU generated from $P$ by grounding all probabilistic
...
\begin{mydef}
	...
	\begin{itemize}
		\item instead of $E_u\subseteq E$, a set of uncertain duration between pairs of
...
through a variety of names. namely Best-first Conflict-Directed Relaxation
...
The Best-first Conflict-Directed Relaxation algorithm was first presented in
\cite{Yu_BCDR_2013} for enumerating the relaxations to a CCTP in best-first
...
\Output{A relaxation $\langle A,R\rangle $ that maximizes $f_p-f_e$.}
\Initialize{}
\Indp{$Cand\leftarrow \langle A,R_e,C_r,C_{cont}\rangle $; the first candidate}\;
{$Q\leftarrow\{Cand\}$; a priority queue that records candidates}\;
...
\Indp
\While{$Q\neq \emptyset$}{
	$Cand\leftarrow $Dequeue$(Q)$\;
	$currCFT\leftarrow$\textsc{UnresolvedConflicts}$(Cand,C)$\;
	\eIf{$currCFT==null$}{
		\eIf{isComplete?$(Cand,U)$}{
			$newCFT\leftarrow$\textsc{ConsistencyCheck}$(cand)$\;
			\eIf{$newCFT==null$}{
				\Return $Cand$\;
			}{
			$C\leftarrow C\cup\{newCFT\}$\;
			$Q\leftarrow Q\cup\{Cand\}$\;
		}			
		}{
			$Q\leftarrow Q\cup$\textsc{ExpandOnVariable}($Cand,U$)\;
		}
		}{
		$Q\leftarrow Q\cup$\textsc{ExpandOnConflict}($Cand,currCFT$)\;
	}
}
\Return $null$\;
...
BCDR starts with an empty candidate in the queue (Line 1). A candidate, $Cand$,
...
Within each loop, BCDR first dequeues the best partial candidate, $Cand$ (Line
6). It checks if $Cand$ resolves all known conflicts (Line 7). If not, an
unresolved conflict $currCFT$ will be returned by function
\textsc{ResolveKnownConflicts?}, which compares the resolved conflicts $C_r$ in
$Cand$ with all known conflicts $C$. $currCFT$ is then used for expanding $Cand$
by function \textsc{ExpandOnConflict} (Line 21). The child candidates of $Cand$,
...
If $Cand$ resolves all known conflicts, BCDR then checks if it is complete,
which means that no more variables can be assigned, by comparing its assignments
and all unassigned variables in the CCTP (Line 9). If $Cand$ is incomplete, BCDR
...
If $Cand$ is complete, BCDR proceeds to check its consistency using function
\textsc{ConsistencyCheck} (Line 10). If no conflict is returned, $Cand$ will be
returned as the best relaxation for the input CCTP (Line 12). If a new conflict,
$newCFT$, is detected by \textsc{ConsistencyCheck}, BCDR will record it in $C$
and put $Cand$ back to the queue for future expansions (Line 14,15), since it
...
\begin{algorithm}[h!]
	...
	\SetKwInput{Algorithm}{Algorithm}
	\Indm
	\Input{A candidate to expand $Cand$ $\langle A,R,C_r,C_{cont}\rangle$ and a
		minimal conflict $currCFT$.}
	\Output{A set of expanded candidates $newCands$.}
	\Initialize{}
	\Indp
	{$newCands\leftarrow \{\}$}; collection of newly generated candidates\;
	{$CFTs\leftarrow C_{cont}\cup \{currCFT\}$; conflicts to be resolved
		continuously}\;
	{$A_{alter}\leftarrow \{\}$}; collection of alternative assignments for the ones in $currCFT$\;
	\Indm
	\Algorithm{}
	\Indp
	\For{$a\in currCFT$}{
	...
	\For{$a_{extend}\in A_{alter}$}{
		\If{\textsc{notCompeting($A,a_{extend}$)}}{
			$Cand_{new}\leftarrow \langle A\cup\{a_{extend}\},R,C_r,C_{cont}\rangle$\;
			$newCands\leftarrow newCands\cup \{Cand_{new}\}$\;
		}
	}
	$E_{relax}\leftarrow$\textsc{ExtractConstraints}$(CFTs)$\;
	$f_{obj}\leftarrow\sum_{e\in E_{relax}}f_e(\Delta e)$\;
	$R_{new}\leftarrow$\textsc{Optimize}$(f_{obj},\langle
	E_{relax}\geq \textbf{0}\rangle )$\;
	\If{$R_{new}\neq null$}{
		$Cand_{new}\leftarrow\langle A,R_{new},C_r,C_{cont}\rangle $\;
		$newCands\leftarrow newCands\cup \{Cand_{new}\}$\;
	} 
	\Return $newCands$\;
...
deactivates the variable and resolves the conflict. For example, assuming that we have to make an additional decision on the imaging method, either using stereo camera or mono camera, for methane seeps site Y. Then for a conflict that involves assignment $IMAGE_Y=STEREO$, given tha we know variable $IMAGE_Y$ has label $MS=Y$, we can resolve the conflict by flipping the assignment to either $IMAGE_Y$ or $MS$: $IMAGE_Y$=$MONO$, $MS$=$X$ or $MS$=$Z$.
...
the relaxation costs of all relaxable constraints, as in (\ref{eqn:rel_obj}).
...
original bounds, as in (\ref{eqn:rel_req}). The resolution constraints in
(\ref{eqn:conf_res}) are added to ensure that all known conflicts are repaired
...
Temporal Problem with Preferences. \cite{Khatib01temporalconstraint}
...
conflict again, as demonstrated in Figure \ref{fig:ExpandOnConflict}.
...
\begin{algorithm}[htb!]
	...
	\SetKwIF{If}{ElseIf}{Else}{if}{then}{else if}{else}{endif}
	\Indm
	\Input{\textbf{A CCTPU $T=\langle P,Q,V,V_r,E,E_u,RE,RE_u,L_p,L_e,f_p,f_e\rangle$.}}
	\Output{Assignments and relaxations $\boldsymbol{\langle A,R_e,R_u\rangle}$ that maximizes $f_p-f_e$.}
	\Initialize{}
	\Indp{$Cand\leftarrow \langle A,R_e,R_u,C_r,C_{cont}\rangle $; the first
		candidate}\;
	{$Q\leftarrow\{Cand\}$; a priority queue that records candidates}\;
	{$C\leftarrow\{\}$; the set of all known conflicts}\;
	{$U\leftarrow V$; the list of unassigned controllable variables}\;
	...
	\While{$Q\neq \emptyset$}{
		...
		\eIf{$currCFT==null$}{
			\eIf{isComplete?$(Cand,U)$}{
				$newCFT\leftarrow$\textbf{\textsc{ControllabilityCheck}}$(Cand)$;\\
				\eIf{$newCFT==null$}{
					\Return $Cand$\;
				}{
				$C\leftarrow C\cup\{newCFT\}$\;
...
$Q\leftarrow$ $Q\cup$\textbf{\textsc{ExpandOnConflict}}($Cand,currCFT$);\\
}
}
\Return $null$\;
...
during reduction. We extend the polynomial time algorithm in
\cite{Vidal99handlingcontingency} with additional procedures for recording
...
\begin{itemize}
	...
	uncontrollable node using the triangular reduction rule. If constraint A is
...
\textsc{fastDCcheck} algorithm in \cite{Morris_astructural} with additional
...
As proved in \cite{Morris_astructural}, an STNU is dynamically controllable if
...
iterative method introduced in \cite{Morris_astructural}. In this example, there
...
\{$B_U+A_L-A_U-B_L$;$-B_L$\},. Making making any one of them non-negative will
...
the implementation in \cite{Morris05temporaldynamic} and will not affect the
correctness of the results. The integration of BCDR with a cubic DC checking
algorithm from \cite{morris2014dynamic}, is expected to further improve
...
from \cite{Fang_AAAI_2014} for grounding probabilistic Simple Temporal Problems
...
\begin{algorithm}[h!]
	...
	\SetKwIF{If}{ElseIf}{Else}{if}{then}{else if}{else}{endif}
	\Indm
	\Input{A cc-pCCTP $\langle P, Q, V, V_r, E, E_d, RE, L_e, L_p, f_p, f_e, \Delta_t,r\Delta_t,f_\Delta\rangle$.}
	\Output{A solution $\langle A,R_e,\Delta'_t,N_{alloc}\rangle$ that maximize
		$f_{p}-f_{e}-f_\Delta$.}
	\Initialize{}
	\Indp{$Cand\leftarrow \langle A,R_{e},\Delta_t,N_{default},C_r\rangle $; the
		first candidate with the default risk allocation}\;
	{$Q\leftarrow\{Cand\}$; a priority queue of candidates}\;
	{$C\leftarrow\{\}$; the set of all known conflicts}\;
	{$U\leftarrow V$; the list of unassigned controllable variables}\;
	\Indm
	\Algorithm{}
	...
	\While{$Q\neq \emptyset$}{
		$Cand\leftarrow $Dequeue$(Q)$\;
		$currCFT\leftarrow$\textsc{UnresolvedConflict}$(Cand,C)$\;
		\eIf{$currCFT==null$}{
			\eIf{isComplete?$(Cand,U)$}{
				$newCFT\leftarrow$\textsc{ControllabilityCheck}$(Cand)$;\\
				\eIf{$newCFT==null$}{
					\Return $Cand$\;
				}{
					$C\leftarrow C\cup\{newCFT\}$\;
			...
			$Q\leftarrow$ $Q\cup$\textsc{\textsc{ExpandOnConflictAndAllocateRisk}}($Cand,currCFT$);\\
		}
	}
	\Return $null$\;
\caption{BCDR-C algorithm for solving cc-pCCTPs}
\label{alg:bcdr_chance_constraints}
\end{algorithm}


We first present an overview of the algorithm that highlights the
modifications, then discuss the risk-allocation and chance constraint
relaxation procedures in detail. The pseudo code of the chance-constrained version of BCDR-C is
presented in Algorithm \ref{alg:bcdr_chance_constraints}. Similar to the
previous two versions, we implement BCDR-C with a priority queue for enumerating
resolutions in best-first order. The algorithm starts with an empty candidate
(Line 1) that has no assignments or relaxations over temporal constraints ($A$ and $R_{e}$) and
chance constraint ($\Delta_{cand}$), and an empty set of resolved conflicts
($C_r$). The candidate is associated with a default risk allocation over all
probabilistic uncertain durations, which is represented by a CCTPU ($N_{default}$).
The initial allocation is computed from a non-linear solver and is conservative
enough to satisfy the chance constraint. The initial candidate is the only element
in the queue before search starts (Line 2).
...
$currCFT$ will be returned by function \textsc{UnresolvedConflict}. The
unresolved conflict is then used for expanding $Cand$ (Line 21). All child
candidates returned by function \textsc{ExpandOnConflictAndAllocateRisk} resolve
$currCFT$ while satisfying the chance constraints. The function also computes
...
If $Cand$ resolves all known conflicts, BCDR-C will proceed to check the
controllability of its grounded CCTPU (function \textsc{ControllabilityCheck},
Line 8). BCDR-C(SC) implements this function with a strong controllability checking algorithm, while BCDR-C(DC)'s function implements dynamic controllability checking. If the grounded network passes the check, $Cand$ will be returned as the best resolution to the
cc-pCCTP (Line 12). Otherwise, a new conflict will be returned by this function
and recorded for expanding candidates (Line 14). $Cand$ will also be added back
...
\begin{problem}[Conflict resolution with chance constraints and probabilistic durations]
	...
	&\phantom{=}	\sum_{rd\in R_d}\textsc{Risk}(lb'_{pj},ub'_{pj}) \leq \Delta'_t, \quad \Delta'_t \in \left[\Delta_t,1\right) \label{eqn:cc}
	\end{align}
...
quite (Line 6). Otherwise, the first solution will be presented to the user. If
...
the RCPSPs in \cite{cui2015optimising}; the rational number approach requires an
...
that only looks at the new conflict $currCFT$  (Algorithm \ref{alg:ExpandOnConflict_greedy}), instead of all conflicts that
...
captured by the variable $Offset$, and the conflict is resolved once the
variable is made zero or negative. If one constraint cannot provide enough
deviation, the procedure will move on to the next one, until $Offset$ is made
...
\begin{algorithm}[htb!]
	...
	\nonl {$Offset\leftarrow$ 0 - \textsc{EvalExp($currCFT$)}};  amount of relaxation that need to be applied for resolving the conflict continuously\;
	\nonl ... ...\\
	\For{$c \in currCFT$}{
		\If{\textsc{isRelaxable($c$)}}{
			$r = \textsc{Min}(Offset,\textsc{RelaxationLimit}(c))$\;
			$R_{new} \leftarrow R_{new} \cup \{\langle c,\textsc{LB}(c)-r,0\rangle \}$ or $\{\langle c,0,\textsc{UB}(c)+r\rangle \}$\;	
			$Offset = Offset - r$\;
		}
		\If{\textsc{$Offset \leq 0$}}{
			$Cand_{new}\leftarrow\langle A,R_{new},C_r,C_{cont}\rangle $\;
			$newCands\leftarrow newCands\cup Cand_{new}$\;
			break;
		}
...
\If{$newCFT==null$}{
	\eIf{\textsc{HasGreedyRelaxation}($cand$)}{
		$cand_{opt} \leftarrow$ \textsc{RefineRelaxation}($cand$)\;
...
algorithm $O(N^4)$ from \cite{Morris_astructural}. It is interesting to observe
...
and more details can be found in \cite{bellei2010transit}. Here we present a
...
Our approach is similar to the frequency-based method proposed in
\cite{bartholdi2012self}, which has each bus observe the preceding and
...
introduced in \cite{wah-xin:06} for checking dynamic controllability, and later
modified by \cite{cui2015optimising} to evaluate the robustness of schedules for
...
\citeauthor{cui2015optimising} \citeyear{cui2015optimising}, the objective of
...
encoding introduced in \cite{cui2015optimising} to compute the maximum
...
graph represents the averaged runtime for one problem over five test runs, and
the error bars indicate the standard deviations of the five runtimes. Similar to
