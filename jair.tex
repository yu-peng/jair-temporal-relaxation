...
\begin{algorithm}[h!]
	...
	\SetKwInput{Algorithm}{Algorithm}
	\Indm
	\Input{A candidate to expand $\mathit{Cand}$ $\langle A,R,C_r,C_{cont}\rangle$ and a
		minimal conflict $\mathit{currConf}$.}
...
\begin{algorithm}[htb!]
	...
	\While{$Q\neq \emptyset$}{
		...
		\eIf{$\mathit{currConf}==null$}{
			\eIf{isComplete?$(\mathit{Cand},U)$}{
				$\mathit{newConf}\leftarrow$\textbf{\textsc{ControllabilityCheck}}$(\mathit{Cand})$;\\
				\eIf{$\mathit{newConf}==null$}{
					\Return $\mathit{Cand}$\;
				}{
				$C\leftarrow C\cup\{\mathit{newConf}\}$\;
...
$Q\leftarrow$ $Q\cup$\textbf{\textsc{ExpandOnConflict}}($\mathit{Cand},\mathit{currConf}$);\\
}
}
\Return $null$\;
...
\begin{algorithm}[h!]
	...
	\Output{A solution $\langle A,R_e,\Delta'_t,N_{alloc}\rangle$ that maximize
		$f_{p}-f_{e}-f_\Delta$.}
	\Initialize{}
	\Indp{$\mathit{Cand}\leftarrow \langle A=\emptyset,R_{e}=\emptyset,\Delta_t,N_{default},C_r=\emptyset\rangle $; the
