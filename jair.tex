...
\maketitle
\jairheading{60}{2017}{99-100}{12/16}{10/17}
\firstpageno{99}
...
suspension of constraints, with the exception of \citeA{lanz2015simple}, which
...
Consider the following example in which an
...
\Indp
\While{$Q\neq \emptyset$}{
	$\mathit{Cand}\leftarrow $\textsc{Dequeue}$(Q)$\;
...
\begin{algorithm}[ht!]	
	...
	\While{$ReductionQ\neq \emptyset$}{
		$\alpha\leftarrow $\textsc{Dequeue}$(ReductionQ)$\;
...
\begin{algorithm}[h!]
	...
	\Output{$Sol$: A valid solution, or $null$ if none exists for the input problem.}
	\Initialize{}
...
with an Intel Core i7-2600 processor and 32GB of RAM. The SNOPT optimizer we used
...
traversal times between survey sites is highly uncertain and difficult to
...
requirement. If no such allocation can be found, it will explain
...
presented in Figure \ref{fig:bcdr_runtime_results}. Each dot in the graph
...
and \citeA{bellei2010transit} present more details about it. Here we present a
...
problem. Assume that three vehicles are running in 8-minute intervals and
...
weakened the headway requirements from the schedule by $\pm$1 minute to allow
...
and both approaches were able to solve all problems within the time limit (30
...
candidates before reaching the optimal relaxations for Instance 135 (825
candidates dequeued), which explains the longer runtime on this problem. Next, we
...
bounded risk. They help users resolve conflicts in temporal problems by
trading off safety margins with performance, which provides flexibility for
